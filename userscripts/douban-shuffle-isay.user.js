...
/**
 * https://greasyfork.org/en/scripts/405863-shuffle-douban-isay/
 * Last Updated: Sept 3, 2020
 * Status: Valid but Not Used
 * Note: 特殊时期规避审查的需求
 */
...
(function () {
  ...
  if (commentContainer) {
    var commentSubmitBtn = document.querySelector(
      '.comment-editor .comment-form-btn'
    );
    ...
    reorderBtn2.style.cssText =
      'height: 30px;line-height: 28px;padding: 0;width: 66px;border-radius: 3px;' +
    ...
    reshareBtns[i].addEventListener('click', function () {
      let timerId = setInterval(() => {
        if (document.querySelector('.reshare-form')) {
          var reshareFooter = document.querySelector(
            '.reshare-form .reshare-footer'
          );
          var reshareSubmit = document.querySelector(
            '.reshare-form .reshare-footer .reshare-btn'
          );
          ...
          reorderBtn3.style.cssText =
            'width: 5em; height: 30px; ling-height: 30px; text-align: center;' +
