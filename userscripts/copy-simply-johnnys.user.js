...
/**
 * https://greasyfork.org/en/scripts/441122-copy-simply-johnny-s/
...
(function () {
  ...
  let donot = [...document.querySelectorAll('script')].filter((n) =>
    /donot\.js$/.test(n.src)
  );
  if (donot.length) {
    donot[0].remove();
  ...
  unsafeWindow.________J &&
    unsafeWindow.________J.doNot({
      selectContent: null,
      captureImages: null,
      useContextMenu: null,
      useKeys: null,
  ...
  if (/johnnys-web\.com/.test(unsafeWindow.location.hostname)) {
    ...
    unsafeWindow.navigator.appVersion =
      '5.0 (iPad; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1';
    ...
    if (post) {
      ...
      var color = getComputedStyle(
        document.querySelector('.blog .section-article .entry__date')
      ).color;
